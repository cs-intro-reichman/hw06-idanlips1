...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		// test blend function
		Color[][] imageOut = read("tinypic.ppm");
		Color[][] image1 = read("ironman.ppm");
		Color[][] news = blend(imageOut, image1, 0.25);
		System.out.println();
		print(news);
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < image.length; i++) {
			...
			for (int j = image[0].length - 1; j >= 0; j--) {
				flippedHorImage[i][count] = image[i][j];
				count++;
			}
	...
	public static Color[][] flippedVertically(Color[][] image) {
		...
		for (int i = image.length - 1; i >= 0; i--) {
			for (int j = 0; j < image[0].length; j++) {
				flippedVerimage[count][j] = image[i][j];
			}
			count++;
		}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height;i++){
			for (int j = 0; j < width;j++){
				newScaled[i][j] = image[(int)(i * ((double)image.length / height))][(int)(j * ((double)image[0].length / width))];
